    ...
    <el-card class="addappmsg-card">
      ...
      >
        <el-form-item label="用户id" prop="usersId">
          <el-input v-model="ruleForm.usersId" placeholder="请输入用户id" disabled></el-input>
        </el-form-item>
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
            ...
            this.$http.post(this.$api.addAppData.addAppData,params,true).then(result=>{
              if(result.data.state=="success"){
                Events.errPrompt(
                  this.$message({
                    message: '恭喜你，应用数据添加成功',
                    type: 'success'
                  })
                );
                this.$router.push({
